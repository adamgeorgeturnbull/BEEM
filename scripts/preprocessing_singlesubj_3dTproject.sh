...
anat=t1_mprage_sag_p2_iso

rest=rest_ep2d_bold_MB8_2mm
...
fslreorient2std ${anat}.nii.gz rest_preproc/${anat}_std.nii.gz
fslreorient2std ${rest}.nii.gz rest_preproc/${rest}_std.nii.gz
fslreorient2std ${fmPA}.nii.gz rest_preproc/${fmPA}_std.nii.gz
fslreorient2std ${fmAP}.nii.gz rest_preproc/${fmAP}_std.nii.gz
...
then
	...
	$afnidir/3dcalc -a ${rest}_std.nii.gz[4..${n_vols}] -expr 'a' -prefix ${rest}_dr.nii.gz -overwrite


	## Extract twelfth volume; this will serve as the "SBRef" for registration

	$afnidir/3dcalc -a ${rest}_std.nii.gz[11] -expr 'a' -prefix ${rest}_SBRef.nii.gz -overwrite
	...
	$fsldir/mcflirt -in ${rest}_dr.nii.gz -out ${rest}_mcf.nii.gz -refvol ${rest}_SBRef.nii.gz -plots -rmsrel
	...
	$fsldir/flirt -in ${fmPA}_std.nii.gz -ref ${rest}_SBRef.nii.gz -out ${fmPA}_mc.nii.gz -dof 6 -interp spline

	$fsldir/flirt -in ${fmAP}_std.nii.gz -ref ${rest}_SBRef.nii.gz -out ${fmAP}_mc.nii.gz -dof 6 -interp spline

	## fslmerge: concatenate the AP and PA single band reference images
	$fsldir/fslmerge -t  fieldmap_restREF.nii.gz ${fmAP}_mc.nii.gz ${fmPA}_mc.nii.gz

	$fsldir/topup --imain=fieldmap_restREF.nii.gz  --datain=../../datain.txt --config=b02b0.cnf --out=my_output

	## topup defaults to spline
	$fsldir/applytopup --imain=${rest}_mcf.nii.gz --inindex=1 --datain=../../datain.txt --topup=my_output --method=jac --out=${rest}_tu.nii.gz

	## Slice timing correction
	## NOTE: --repetition flag does not do anything when specifying custom slice timing acquisition
	echo Slice time correction ${rest}.nii.gz 

	$fsldir/slicetimer -i ${rest}_tu.nii.gz -o ${rest}_st.nii.gz --tcustom=../../slicetiming.txt
...
then
	...
	$afnidir/3dSkullStrip -input ${anat}_std.nii.gz -o_ply ${anat}_surf.nii.gz -overwrite
	$afnidir/3dcalc -a ${anat}_std.nii.gz -b ${anat}_surf.nii.gz -expr 'a*step(b)' -prefix ${anat}_std_brain.nii.gz -overwrite
...
then
	...
	cp ${rest}_SBRef.nii.gz ${reg_dir}/example_func.nii.gz


	## 1. Copy required images into reg directory
	### copy anatomical
	cp ${anat}_std_brain.nii.gz ${reg_dir}/highres.nii.gz
	cp ${anat}_std.nii.gz ${reg_dir}/highres_head.nii.gz
	### copy standard
...
then
	...
	cp ${rest}_pp_mask.nii.gz ${segment_dir}/global_mask.nii.gz
	...
	$afnidir/3dmaskave -mask ${segment_dir}/global_mask.nii.gz -quiet ${rest}_pp.nii.gz > ${nuisance_dir}/global.1D -overwrite

	## 3. csf
	echo "Extracting signal from csf for ${subject}"
	$afnidir/3dmaskave -mask ${segment_dir}/csf_mask.nii.gz -quiet ${rest}_pp.nii.gz > ${nuisance_dir}/csf.1D -overwrite

	## 4. wm
	echo "Extracting signal from white matter for ${subject}"
	$afnidir/3dmaskave -mask ${segment_dir}/wm_mask.nii.gz -quiet ${rest}_pp.nii.gz > ${nuisance_dir}/wm.1D -overwrite
...
## No spatial smoothing:
	$afnidir/3dTproject -input ${rest}_pp.nii.gz -prefix ${rest}_pp_9p.nii.gz -polort 2 -ort ${nuisance_dir}/global.1D ${nuisance_dir}/wm.1D ${nuisance_dir}/csf.1D ${rest}_mcf.nii.gz.par -automask -overwrite -verb
	$afnidir/3dTproject -input ${rest}_pp.nii.gz -prefix ${rest}_pp_9p_tf.nii.gz -polort 2 -ort ${nuisance_dir}/global.1D ${nuisance_dir}/wm.1D ${nuisance_dir}/csf.1D ${rest}_mcf.nii.gz.par -passband $hp $lp -automask -overwrite -verb

## sdev maps
	$afnidir/3dTstat -stdev -prefix ${rest}_pp_stdev.nii.gz ${rest}_pp.nii.gz  -overwrite

	$afnidir/3dcalc -a ${rest}_pp_9p_stdev.nii.gz -b  $base_data -expr 'b/a' -prefix ${rest}_pp_9p_gfactor.nii.gz -overwrite
	$afnidir/3dcalc -a ${rest}_pp_9p_tf_stdev.nii.gz -b  $base_data_tf -expr 'b/a' -prefix ${rest}_pp_9p_tf_gfactor.nii.gz -overwrite


## Spatial smoothing:
	$afnidir/3dTproject -input ${rest}_pp.nii.gz -prefix ${rest}_pp_9p_sm.nii.gz -polort 2 -ort ${nuisance_dir}/global.1D ${nuisance_dir}/wm.1D ${nuisance_dir}/csf.1D ${rest}_mcf.nii.gz.par -blur $FWHM -automask -overwrite -verb
	$afnidir/3dTproject -input ${rest}_pp.nii.gz -prefix ${rest}_pp_9p_tf_sm.nii.gz -polort 2 -ort ${nuisance_dir}/global.1D ${nuisance_dir}/wm.1D ${nuisance_dir}/csf.1D ${rest}_mcf.nii.gz.par -passband $hp $lp -blur $FWHM -automask -overwrite -verb

## sdev maps
	$afnidir/3dTstat -stdev -prefix ${rest}_pp_9p_sm_stdev.nii.gz ${rest}_pp_9p_sm.nii.gz  -overwrite
	$afnidir/3dTstat -stdev -prefix ${rest}_pp_9p_tf_sm_stdev.nii.gz ${rest}_pp_9p_tf_sm.nii.gz -overwrite
